...
test('匯出簽收表：欄位與資料正確', async () => {
  const csv = await (await fetch(`${base}/api/signoff.csv?q=SOPW1`)).text();
  const header = csv.split('\n')[0];
  for (const h of ['編號', '專案名稱', '產品名稱', '兌換連結', '密碼', '期貨帳號', '購買人姓名', '身份證字號', '戶籍地址', '面額', '單位', '營業員', '發送狀態']) {
    assert.ok(header.includes(h), `簽收表應含欄位 ${h}`);
  }
